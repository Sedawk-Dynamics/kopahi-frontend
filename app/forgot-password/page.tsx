...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus("sent");
    setMessage(`If an account exists for ${trimmed}, you'll get a reset link within a minute.`);
  };
    ...
    <main className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100 flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-gray-100 p-8 md:p-10">
        <Link href="/" className="flex items-center gap-2 mb-8">
          <Image src="/Logo1.png" alt="Kopahi" width={48} height={48} className="object-contain" />
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          {message && (
            <p
              role="status"
              aria-live="polite"
              className={`text-sm text-center ${status === "error" ? "text-red-600" : "text-green-700"}`}
            >
              {message}
            </p>
          )}
